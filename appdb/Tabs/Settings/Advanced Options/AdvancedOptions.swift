...
class AdvancedOptions: TableViewController {
    ...
    fileprivate func getSections() -> [StaticSection] {
        var sections = [StaticSection(header: .title("Signing".localized()), rows: [
            ...
            StaticRow(text: "Use Revoked Certificates".localized(), accessory: .switchToggle(value: Preferences.disableRevocationChecks) { newValue in
                API.setConfiguration(params: [.useRevokedCerts: newValue ? "yes" : "no"], success: {}, fail: { _ in })
        ...
        StaticSection(header: .title("Device Tools".localized()), rows: [
